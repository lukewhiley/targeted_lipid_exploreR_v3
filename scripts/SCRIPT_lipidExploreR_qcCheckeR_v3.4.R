...
#ensure onlt .rda files are in filelist
...
master_list$data$pp_mvFilter <- master_list$data$area_sorted

for(idx_data in names(master_list$data$area_sorted)){
  master_list$data$pp_mvFilter[[idx_data]] <- master_list$data$pp_mvFilter[[idx_data]] %>%
...
for(idx_data in names(master_list$data$pp_mvFilter)){
  master_list$process_lists$mvLipids[[paste0(idx_data,".peakArea<5000[LOD]")]] <- colSums(
    x= (master_list$data$pp_mvFilter[[idx_data]] %>%
          select(-contains("sample")) %>%
          as.matrix()) <5000, na.rm = T)
  ...
  master_list$process_lists$mvLipids[[paste0(idx_data, ".keepLipid[Plate]")]][which(
    master_list$process_lists$mvLipids[[paste0(idx_data,".totalMissingValues")]] > (nrow(master_list$data$pp_mvFilter[[idx_data]]) * 0.5)
  )] <- 0
...
master_list$process_lists$mvLipids[[paste0("allPlates.peakArea<5000[LOD]")]] <- colSums(
  x= (master_list$data$pp_mvFilter %>%
        bind_rows() %>%
        select(-contains("sample")) %>%
        as.matrix()) <5000, na.rm = T)
...
master_list$process_lists$mvLipids[[paste0("allPlates.keepLipid[all]")]] [which(
  master_list$process_lists$mvLipids[[paste0("allPlates.totalMissingValues")]] > (nrow(bind_rows(master_list$data$pp_mvFilter)) * 0.5)
)] <- 0

# add fail.filter column if lipid failed for a plate or for all plates
master_list$process_lists$mvLipids[["lipidKeep"]] <- 1
master_list$process_lists$mvLipids[["lipidKeep"]][which(
...
for(idx_data in names(master_list$data$pp_mvFilter)){
  master_list$data$pp_mvFilter[[idx_data]] <- master_list$data$pp_mvFilter[[idx_data]] %>%
    select(-any_of(
      (master_list$process_lists$mvLipids %>% filter(lipidKeep == 0))[["lipid"]]
    ))
  
  master_list$data$pp_mvFilter[[idx_data]]$sample_data_source <- "pp_mvFiltered"
}
...
for(idx_data in names(master_list$data$pp_mvFilter)){
  master_list$data$pp_impute[[idx_data]] <- master_list$environment$user_functions$impute_data$value(
    FUNC_data = master_list$data$pp_mvFilter[[idx_data]],
    FUNC_metabolite_list = master_list$data$pp_mvFilter[[idx_data]] %>% 
...
master_list$data$statTarget_mvFilter <- list()
#loop to back into plate elements of list
for(idx_data in unique(statTarget_data$sample_plate_id)){
  master_list$data$statTarget_mvFilter[[idx_data]] <- statTarget_data %>%
    filter(sample_plate_id == idx_data) %>%
    select(
      contains("sample"),
  ...
  master_list$data$statTarget_mvFilter[[idx_data]]$sample_data_source <- "statTarget_mvFilter"
...
for(idx_data in names(master_list$data$statTarget_mvFilter)){
  loop_data <- master_list$environment$user_functions$conc_calc$value(
    FUNC_data = master_list$data$statTarget_mvFilter[[idx_data]],
    FUNC_SIL_list = master_list$data$statTarget_mvFilter[[idx_data]] %>% 
      select(-contains("sample")) %>% select(contains("SIL")) %>% names(),
    FUNC_SIL_guide = master_list$templates$SIL_guide,
    FUNC_conc_guide = master_list$templates$conc_guide
  )
  ...
  master_list$data$statTarget_response[[idx_data]]$sample_data_source <- "statTargetResponse"; master_list$data$statTarget_concentration[[idx_data]]$sample_data_source <- "postFilterStatTarget"
}
...
for(idx_data in names(master_list$data$statTarget_concentration)){
  master_list$data$statTarget_final[[idx_data]] <- master_list$data$statTarget_concentration[[idx_data]] %>%
    select(
      contains("sample"),
...
tempMatrix <- bind_rows(master_list$data$statTarget_concentration) %>%
...
master_list$data$statTarget_final_allPlates <- list(
  bind_rows(master_list$data$statTarget_concentration) %>%
    select(contains("sample")) %>%
    left_join(.,
              tempMatrix %>% 
                rownames_to_column("sample_name"),
...
master_list$pca$peakArea$sample_type <- master_list$environment$user_functions$pca$value(
  FUNC_data =  bind_rows(
    ...
    mutate(sample_data_source = factor(
      ...
      levels = c("peakArea",
                 "concentration",
                 "postFilterConcentration",
                 "postFilterStatTarget"),
      ordered = TRUE
    )) %>%
    select(-contains("SIL")), 
  FUNC_HEADER_run_order = "sample_run_index",
...
master_list$summary_tables$odsAreaOverview <- rbind(
  ...
  c("total.SilInternalStandards", bind_rows(master_list$data$area_sorted) %>% select(contains("SIL")) %>% ncol()),
  c("", ""),
  c("TAB_DESCRIPTION:", ""),
  ...
  c("DATA.statTargetConcentration", "concentration data that has undergone pre-processing including signalDrift | batch correction using the statTarget r package")
  ) %>%
  as_tibble() 
...
openxlsx::write.xlsx(
  file = paste0(master_list$project_details$project_dir, 
                ...
                "_lipidData_qcCeheckeR_v3.5.xlsx"),
  overwrite = TRUE,
  x = list(
    ...
    "QC.lipidQcRsd" = (master_list$process_lists$rsd_filter %>% t() %>% as.data.frame() %>% rownames_to_column() %>% setNames(nm = .[1,]))[-1,] %>% 
      mutate(across(-lipid, as.numeric)) %>%
      separate_wider_delim(
        cols = lipid,
        delim = ".",
...
rmarkdown::render(input = paste0(master_list$project_details$project_dir, "/html_report/lipid_exploreR_report_templatev3.5.R"),
                  ...
                  output_file = paste0(Sys.Date(), "_", master_list$project_details$project_name, "_lipidExploreR_qcCheckeR_report_v3.35.html")
)

browseURL(url = paste0(master_list$project_details$project_dir, 
                       "/html_report/",
                       Sys.Date(), "_", master_list$project_details$project_name, "_lipidExploreR_qcCheckeR_report_v3.35.html")
)
