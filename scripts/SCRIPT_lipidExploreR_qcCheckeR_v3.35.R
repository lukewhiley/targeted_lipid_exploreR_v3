...
### 4. Select data features that are common for all plates ------------
...
### 5. Impute missing values [min/2 imputation (missing assumed < LOD)] -----------------------------------------------------
...
### 6. Calculate response ratio and concentration calculations  ------------------------------------------------------
...
#.----
  ...
  #### e. interPlate concentration (missing value filtered | impute | %rsd filtered) ---------------------------
  master_list$summary_tables$area_preProcessed <- master_list$summary_tables$area_preProcessed %>%
    left_join(
      rbind(
        c("total samples", nrow(master_list$data$area_impute_concentration_rsd_all)),
        c("study samples", nrow(master_list$data$area_impute_concentration_rsd_all %>% filter(sample_type_factor == "sample"))),
        c("ltr sample", nrow(master_list$data$area_impute_concentration_rsd_all %>% filter(sample_type_factor == "ltr"))),
        c("vltr sample", nrow(master_list$data$area_impute_concentration_rsd_all %>% filter(sample_type_factor == "vltr"))),
        c("sltr sample", nrow(master_list$data$area_impute_concentration_rsd_all %>% filter(sample_type_factor == "sltr"))),
        c("pqc sample", nrow(master_list$data$area_impute_concentration_rsd_all %>% filter(sample_type_factor == "pqc"))),
        c("total lipid features", ncol(master_list$data$area_impute_concentration_rsd_all %>% select(-contains("sample"), - contains("SIL")))),
        c("total SIL Int. Stds", NA),
        c("zero values (lipid features)", length(which(master_list$data$area_impute_concentration_rsd_all %>% select(-contains("sample"), -contains("SIL")) ==0))),
        c("zero values (SIL Int.Stds)", NA),
        c("NA values (lipid features)", length(which(is.na(as.matrix(master_list$data$area_impute_concentration_rsd_all %>% select(-contains("sample"), -contains("SIL"))))))),
        c("NA values (SIL Int.Stds)", NA),
        c("Inf values (lipid features)", length(which(is.infinite(as.matrix(master_list$data$area_impute_concentration_rsd_all %>% select(-contains("sample"), -contains("SIL"))))))),
        c("Inf values (SIL Int.Stds)", NA),
        c("NaN values (lipid features)", length(which(is.nan(as.matrix(master_list$data$area_impute_concentration_rsd_all %>% select(-contains("sample"), -contains("SIL"))))))),
        c("Nan values (SIL Int.Stds)", NA)
      ) %>%
        as_tibble() %>%
        rename(metric = V1,
               acrossAllPlatesBatches = V2),
      by = "metric"
    ) %>%
    filter(!grepl("SIL", metric))
  #change column to numeric
  master_list$summary_tables$area_preProcessed$acrossAllPlatesBatches <- as.numeric(master_list$summary_tables$area_preProcessed$acrossAllPlatesBatches)
  
  #### f. totalSummary ----
  ### 1. dataSetSummary ---------------------------------------
 master_list$summary_tables$peakAreaOverview <- left_join(
   master_list$summary_tables$area_peakArea,
   master_list$summary_tables$area_concentration,
   by = "metric"),
   master_list$summary_tables$area_preProcessed
 )
    master_list$summary_tables$area_peakArea %>%
    select(metric)
  ...
  ### 2. filterSummary -------------------------
  #### a. missingValue filter summary --------------
  ##### [i]. samples -------------------------
  master_list$summary_tables$area_mv_filter_samples <- select(master_list$data$area_sorted[[1]], sample_name)
  
  for(idx_data in names(master_list$data$area_sorted)){
    lipid_data <- master_list$data$area_sorted[[idx_data]] %>% 
      select(-contains("sample")) %>%
      select(-contains("SIL")) %>%
      as.matrix()
    lipid_data[lipid_data<5000] <- NA
    lipid_data[is.infinite(lipid_data)] <- NA
    lipid_data[is.nan(lipid_data)] <- NA
    master_list$summary_tables$area_mv_filter_samples[["sample_plate_id"]] <- idx_data
    master_list$summary_tables$area_mv_filter_samples[["%_missing_values"]] <- (100/ncol(lipid_data))*rowSums(is.na(lipid_data))
  }
  
  master_list$summary_tables$area_mv_filter_samples[["sample_included"]] <- "PASS"
  master_list$summary_tables$area_mv_filter_samples[["sample_included"]][which(master_list$summary_tables$area_mv_filter_samples$`%_missing_values`> 50)] <- "FAIL"
  
  ##### [ii]. lipid Features-------------------------
  
  master_list$summary_tables$area_mv_filter_lipids <- tibble(
    lipid = master_list$data$area_sorted[[1]] %>% select(-contains("sample")) %>% names()
  )
  
  for(idx_data in names(master_list$data$area_sorted)){
    #remove failed samples
    sample_removed_list <- filter(master_list$summary_tables$area_mv_filter_samples, sample_included == "FAIL")[["sample_name"]]
    #created lipid loop data
    lipid_data <- master_list$data$area_sorted[[idx_data]] %>%
      filter(!sample_name %in% sample_removed_list) %>% 
      filter(sample_type_factor == "sample") %>%
      select(-contains("sample")) %>%
      as.matrix()
    lipid_data[lipid_data<5000] <- NA
    lipid_data[is.infinite(lipid_data)] <- NA
    lipid_data[is.nan(lipid_data)] <- NA
    master_list$summary_tables$area_mv_filter_lipids[[paste0(idx_data, "_sample")]] <- (100/nrow(lipid_data))*colSums(is.na(lipid_data))
    #for qc
    lipid_data <- master_list$data$area_sorted[[idx_data]] %>%
      filter(!sample_name %in% sample_removed_list) %>% 
    ...
    lipid_data[is.infinite(lipid_data)] <- NA
    lipid_data[is.nan(lipid_data)] <- NA
    master_list$summary_tables$area_mv_filter_lipids[[paste0(idx_data, "_", tolower(master_list$project_details$qc_type))]] <- (100/nrow(lipid_data))*colSums(is.na(lipid_data))
  }
  
  #add max_% missing column
  master_list$summary_tables$area_mv_filter_lipids[["max_%_missing"]] <- master_list$summary_tables$area_mv_filter_lipids %>%
    select_if(., is.numeric) %>%
    apply(1, max, na.rm=TRUE)
  
  #add pass/fail column
  master_list$summary_tables$area_mv_filter_lipids[["feature_included"]] <- "PASS"
  master_list$summary_tables$area_mv_filter_lipids[["feature_included"]][which(master_list$summary_tables$area_mv_filter_lipids$`max_%_missing`>50)] <- "FAIL"

#### b. RSD filter -----------------------------------
#add max_rsd column
master_list$process_lists$area_rsd_filter[["max_rsd"]] <- master_list$process_lists$area_rsd_filter %>%
  select_if(., is.numeric) %>%
  apply(1, max, na.rm=TRUE)

#add pass/fail column
master_list$process_lists$area_rsd_filter[["feature_included"]] <- "PASS"
master_list$process_lists$area_rsd_filter[["feature_included"]][which(master_list$process_lists$area_rsd_filter$max_rsd>30)] <- "FAIL"
...
## SummaryReports ------------------------ 
### 1. dataSetSummary ---------------------------------------
metric =  c("total samples", "study samples", "ltr sample", "vltr sample", "sltr sample", "pqc sample", "total lipid features", "total SIL Int. Stds", "zero values (lipid features)", "zero values (SIL Int.Stds)", "NA values (lipid features)", "NA values (SIL Int.Stds)", "Inf values (lipid features)", "Inf values (SIL Int.Stds)", "NaN values (lipid features)",  "Nan values (SIL Int.Stds)")
#### a.. PeakArea ---------------------------
master_list$summary_tables$statTarget_peakArea <- tibble(metric = metric)
for(idx_data in names(master_list$data$statTarget_sorted)){
  
  master_list$summary_tables$statTarget_peakArea <- left_join(
    master_list$summary_tables$statTarget_peakArea,
    rbind(
      c("total samples", nrow(master_list$data$statTarget_sorted[[idx_data]])),
      c("study samples", nrow(master_list$data$statTarget_sorted[[idx_data]] %>% filter(sample_type_factor == "sample"))),
      c("ltr sample", nrow(master_list$data$statTarget_sorted[[idx_data]] %>% filter(sample_type_factor == "ltr"))),
      c("vltr sample", nrow(master_list$data$statTarget_sorted[[idx_data]] %>% filter(sample_type_factor == "vltr"))),
      c("sltr sample", nrow(master_list$data$statTarget_sorted[[idx_data]] %>% filter(sample_type_factor == "sltr"))),
      c("pqc sample", nrow(master_list$data$statTarget_sorted[[idx_data]] %>% filter(sample_type_factor == "pqc"))),
      c("total lipid features", ncol(master_list$data$statTarget_sorted[[idx_data]] %>% select(-contains("sample"), - contains("SIL")))),
      c("total SIL Int. Stds", ncol(master_list$data$statTarget_sorted[[idx_data]] %>% select(contains("SIL")))),
      c("zero values (lipid features)", length(which(master_list$data$statTarget_sorted[[idx_data]] %>% select(-contains("sample"), -contains("SIL")) ==0))),
      c("zero values (SIL Int.Stds)", length(which(master_list$data$statTarget_sorted[[idx_data]] %>% select(contains("SIL")) ==0))),
      c("NA values (lipid features)", length(which(is.na(as.matrix(master_list$data$statTarget_sorted[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("NA values (SIL Int.Stds)", length(which(is.na(as.matrix(master_list$data$statTarget_sorted[[idx_data]] %>% select(contains("SIL"))))))),
      c("Inf values (lipid features)", length(which(is.infinite(as.matrix(master_list$data$statTarget_sorted[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("Inf values (SIL Int.Stds)", length(which(is.infinite(as.matrix(master_list$data$statTarget_sorted[[idx_data]] %>% select(contains("SIL"))))))),
      c("NaN values (lipid features)", length(which(is.nan(as.matrix(master_list$data$statTarget_sorted[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("Nan values (SIL Int.Stds)", length(which(is.nan(as.matrix(master_list$data$statTarget_sorted[[idx_data]] %>% select(contains("SIL")))))))
    ) %>%
      as_tibble() %>%
      rename(metric = V1,
             !! idx_data := V2),
  ...
  #change column to numeric
  master_list$summary_tables$statTarget_peakArea[[idx_data]] <- as.numeric(master_list$summary_tables$statTarget_peakArea[[idx_data]])
}


#### b. peakResponse (all lipids) -----------------
master_list$summary_tables$statTarget_response <- tibble(metric = metric)
for(idx_data in names(master_list$data$statTarget_response)){
  
  master_list$summary_tables$statTarget_response <- left_join(
    master_list$summary_tables$statTarget_response,
    rbind(
      c("total samples", nrow(master_list$data$statTarget_response[[idx_data]])),
      c("study samples", nrow(master_list$data$statTarget_response[[idx_data]] %>% filter(sample_type_factor == "sample"))),
      c("ltr sample", nrow(master_list$data$statTarget_response[[idx_data]] %>% filter(sample_type_factor == "ltr"))),
      c("vltr sample", nrow(master_list$data$statTarget_response[[idx_data]] %>% filter(sample_type_factor == "vltr"))),
      c("sltr sample", nrow(master_list$data$statTarget_response[[idx_data]] %>% filter(sample_type_factor == "sltr"))),
      c("pqc sample", nrow(master_list$data$statTarget_response[[idx_data]] %>% filter(sample_type_factor == "pqc"))),
      c("total lipid features", ncol(master_list$data$statTarget_response[[idx_data]] %>% select(-contains("sample"), - contains("SIL")))),
      c("total SIL Int. Stds", NA),
      c("zero values (lipid features)", length(which(master_list$data$statTarget_response[[idx_data]] %>% select(-contains("sample"), -contains("SIL")) ==0))),
      c("zero values (SIL Int.Stds)", NA),
      c("NA values (lipid features)", length(which(is.na(as.matrix(master_list$data$statTarget_response[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("NA values (SIL Int.Stds)", NA),
      c("Inf values (lipid features)", length(which(is.infinite(as.matrix(master_list$data$statTarget_response[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("Inf values (SIL Int.Stds)", NA),
      c("NaN values (lipid features)", length(which(is.nan(as.matrix(master_list$data$statTarget_response[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("Nan values (SIL Int.Stds)", NA)
    ) %>%
      as_tibble() %>%
      rename(metric = V1,
    ...
    by = "metric") %>%
    filter(!grepl("SIL", metric))
  #cange column to numeric
  master_list$summary_tables$statTarget_response[[idx_data]] <- as.numeric(master_list$summary_tables$statTarget_response[[idx_data]])
}

#### c. concentration (all lipids) --------------
master_list$summary_tables$statTarget_concentration <- tibble(metric = metric)
for(idx_data in names(master_list$data$statTarget_concentration)){
  
  master_list$summary_tables$statTarget_concentration <- left_join(
    master_list$summary_tables$statTarget_concentration,
    rbind(
      c("total samples", nrow(master_list$data$statTarget_concentration[[idx_data]])),
      c("study samples", nrow(master_list$data$statTarget_concentration[[idx_data]] %>% filter(sample_type_factor == "sample"))),
      c("ltr sample", nrow(master_list$data$statTarget_concentration[[idx_data]] %>% filter(sample_type_factor == "ltr"))),
      c("vltr sample", nrow(master_list$data$statTarget_concentration[[idx_data]] %>% filter(sample_type_factor == "vltr"))),
      c("sltr sample", nrow(master_list$data$statTarget_concentration[[idx_data]] %>% filter(sample_type_factor == "sltr"))),
      c("pqc sample", nrow(master_list$data$statTarget_concentration[[idx_data]] %>% filter(sample_type_factor == "pqc"))),
      c("total lipid features", ncol(master_list$data$statTarget_concentration[[idx_data]] %>% select(-contains("sample"), - contains("SIL")))),
      c("total SIL Int. Stds", NA),
      c("zero values (lipid features)", length(which(master_list$data$statTarget_concentration[[idx_data]] %>% select(-contains("sample"), -contains("SIL")) ==0))),
      c("zero values (SIL Int.Stds)", NA),
      c("NA values (lipid features)", length(which(is.na(as.matrix(master_list$data$statTarget_concentration[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("NA values (SIL Int.Stds)", NA),
      c("Inf values (lipid features)", length(which(is.infinite(as.matrix(master_list$data$statTarget_concentration[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("Inf values (SIL Int.Stds)", NA),
      c("NaN values (lipid features)", length(which(is.nan(as.matrix(master_list$data$statTarget_concentration[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("Nan values (SIL Int.Stds)", NA)
    ) %>%
      as_tibble() %>%
      rename(metric = V1,
    ...
    filter(!grepl("SIL", metric))
  #change column to numeric
  master_list$summary_tables$statTarget_concentration[[idx_data]] <- as.numeric(master_list$summary_tables$statTarget_concentration[[idx_data]])
}

#### d. intraPlate concentration (missing value filtered | impute | %rsd filtered) ---------------------------
master_list$summary_tables$statTarget_preProcessed <- tibble(metric = metric)
for(idx_data in names(master_list$data$statTarget_impute_concentration_rsd)){
  
  master_list$summary_tables$statTarget_preProcessed <- left_join(
    master_list$summary_tables$statTarget_preProcessed,
    rbind(
      c("total samples", nrow(master_list$data$statTarget_impute_concentration_rsd[[idx_data]])),
      c("study samples", nrow(master_list$data$statTarget_impute_concentration_rsd[[idx_data]] %>% filter(sample_type_factor == "sample"))),
      c("ltr sample", nrow(master_list$data$statTarget_impute_concentration_rsd[[idx_data]] %>% filter(sample_type_factor == "ltr"))),
      c("vltr sample", nrow(master_list$data$statTarget_impute_concentration_rsd[[idx_data]] %>% filter(sample_type_factor == "vltr"))),
      c("sltr sample", nrow(master_list$data$statTarget_impute_concentration_rsd[[idx_data]] %>% filter(sample_type_factor == "sltr"))),
      c("pqc sample", nrow(master_list$data$statTarget_impute_concentration_rsd[[idx_data]] %>% filter(sample_type_factor == "pqc"))),
      c("total lipid features", ncol(master_list$data$statTarget_impute_concentration_rsd[[idx_data]] %>% select(-contains("sample"), - contains("SIL")))),
      c("total SIL Int. Stds", NA),
      c("zero values (lipid features)", length(which(master_list$data$statTarget_impute_concentration_rsd[[idx_data]] %>% select(-contains("sample"), -contains("SIL")) ==0))),
      c("zero values (SIL Int.Stds)", NA),
      c("NA values (lipid features)", length(which(is.na(as.matrix(master_list$data$statTarget_impute_concentration_rsd[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("NA values (SIL Int.Stds)", NA),
      c("Inf values (lipid features)", length(which(is.infinite(as.matrix(master_list$data$statTarget_impute_concentration_rsd[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("Inf values (SIL Int.Stds)", NA),
      c("NaN values (lipid features)", length(which(is.nan(as.matrix(master_list$data$statTarget_impute_concentration_rsd[[idx_data]] %>% select(-contains("sample"), -contains("SIL"))))))),
      c("Nan values (SIL Int.Stds)", NA)
    ) %>%
      as_tibble() %>%
      rename(metric = V1,
    ...
    filter(!grepl("SIL", metric))
  #change column to numeric
  master_list$summary_tables$statTarget_preProcessed[[idx_data]] <- as.numeric(master_list$summary_tables$statTarget_preProcessed[[idx_data]])
}

#### e. interPlate concentration (missing value filtered | impute | %rsd filtered) ---------------------------
master_list$summary_tables$statTarget_preProcessed <- master_list$summary_tables$statTarget_preProcessed %>%
  left_join(
    rbind(
      c("total samples", nrow(master_list$data$statTarget_impute_concentration_rsd_all)),
      c("study samples", nrow(master_list$data$statTarget_impute_concentration_rsd_all %>% filter(sample_type_factor == "sample"))),
      c("ltr sample", nrow(master_list$data$statTarget_impute_concentration_rsd_all %>% filter(sample_type_factor == "ltr"))),
      c("vltr sample", nrow(master_list$data$statTarget_impute_concentration_rsd_all %>% filter(sample_type_factor == "vltr"))),
      c("sltr sample", nrow(master_list$data$statTarget_impute_concentration_rsd_all %>% filter(sample_type_factor == "sltr"))),
      c("pqc sample", nrow(master_list$data$statTarget_impute_concentration_rsd_all %>% filter(sample_type_factor == "pqc"))),
      c("total lipid features", ncol(master_list$data$statTarget_impute_concentration_rsd_all %>% select(-contains("sample"), - contains("SIL")))),
      c("total SIL Int. Stds", NA),
      c("zero values (lipid features)", length(which(master_list$data$statTarget_impute_concentration_rsd_all %>% select(-contains("sample"), -contains("SIL")) ==0))),
      c("zero values (SIL Int.Stds)", NA),
      c("NA values (lipid features)", length(which(is.na(as.matrix(master_list$data$statTarget_impute_concentration_rsd_all %>% select(-contains("sample"), -contains("SIL"))))))),
      c("NA values (SIL Int.Stds)", NA),
      c("Inf values (lipid features)", length(which(is.infinite(as.matrix(master_list$data$statTarget_impute_concentration_rsd_all %>% select(-contains("sample"), -contains("SIL"))))))),
      c("Inf values (SIL Int.Stds)", NA),
      c("NaN values (lipid features)", length(which(is.nan(as.matrix(master_list$data$statTarget_impute_concentration_rsd_all %>% select(-contains("sample"), -contains("SIL"))))))),
      c("Nan values (SIL Int.Stds)", NA)
    ) %>%
      as_tibble() %>%
      rename(metric = V1,
  ...
  filter(!grepl("SIL", metric))
#change column to numeric
master_list$summary_tables$statTarget_preProcessed$acrossAllPlatesBatches <- as.numeric(master_list$summary_tables$statTarget_preProcessed$acrossAllPlatesBatches)

### 2. filterSummary -------------------------
#### a. missingValue filter summary --------------
##### [i]. samples -------------------------
master_list$summary_tables$statTarget_mv_filter_samples <- select(master_list$data$statTarget_sorted[[1]], sample_name)

for(idx_data in names(master_list$data$statTarget_sorted)){
  lipid_data <- master_list$data$statTarget_sorted[[idx_data]] %>% 
    select(-contains("sample")) %>%
    select(-contains("SIL")) %>%
    as.matrix()
  lipid_data[lipid_data<5000] <- NA
  lipid_data[is.infinite(lipid_data)] <- NA
  lipid_data[is.nan(lipid_data)] <- NA
  master_list$summary_tables$statTarget_mv_filter_samples[["sample_plate_id"]] <- idx_data
  master_list$summary_tables$statTarget_mv_filter_samples[["%_missing_values"]] <- (100/ncol(lipid_data))*rowSums(is.na(lipid_data))
}

master_list$summary_tables$statTarget_mv_filter_samples[["sample_included"]] <- "PASS"
master_list$summary_tables$statTarget_mv_filter_samples[["sample_included"]][which(master_list$summary_tables$statTarget_mv_filter_samples$`%_missing_values`> 50)] <- "FAIL"

##### [ii]. lipid Features-------------------------

master_list$summary_tables$statTarget_mv_filter_lipids <- tibble(
  lipid = master_list$data$statTarget_sorted[[1]] %>% select(-contains("sample")) %>% names()
)

for(idx_data in names(master_list$data$statTarget_sorted)){
  #remove failed samples
  sample_removed_list <- filter(master_list$summary_tables$statTarget_mv_filter_samples, sample_included == "FAIL")[["sample_name"]]
  #created lipid loop data
  lipid_data <- master_list$data$statTarget_sorted[[idx_data]] %>%
    filter(!sample_name %in% sample_removed_list) %>% 
    filter(sample_type_factor == "sample") %>%
  ...
  lipid_data[is.infinite(lipid_data)] <- NA
  lipid_data[is.nan(lipid_data)] <- NA
  master_list$summary_tables$statTarget_mv_filter_lipids[[paste0(idx_data, "_sample")]] <- (100/nrow(lipid_data))*colSums(is.na(lipid_data))
  #for qc
  lipid_data <- master_list$data$statTarget_sorted[[idx_data]] %>%
    filter(!sample_name %in% sample_removed_list) %>% 
    filter(sample_type == "qc") %>%
  ...
  lipid_data[is.infinite(lipid_data)] <- NA
  lipid_data[is.nan(lipid_data)] <- NA
  master_list$summary_tables$statTarget_mv_filter_lipids[[paste0(idx_data, "_", tolower(master_list$project_details$qc_type))]] <- (100/nrow(lipid_data))*colSums(is.na(lipid_data))
}

#add max_% missing column
master_list$summary_tables$statTarget_mv_filter_lipids[["max_%_missing"]] <- master_list$summary_tables$statTarget_mv_filter_lipids %>%
  select_if(., is.numeric) %>%
  apply(1, max, na.rm=TRUE)

#add pass/fail column
master_list$summary_tables$statTarget_mv_filter_lipids[["feature_included"]] <- "PASS"
master_list$summary_tables$statTarget_mv_filter_lipids[["feature_included"]][which(master_list$summary_tables$statTarget_mv_filter_lipids$`max_%_missing`>50)] <- "FAIL"

#### b. RSD filter -----------------------------------
#add max_rsd column
master_list$process_lists$statTarget_rsd_filter[["max_rsd"]] <- master_list$process_lists$statTarget_rsd_filter %>%
  select_if(., is.numeric) %>%
  apply(1, max, na.rm=TRUE)

#add pass/fail column
master_list$process_lists$statTarget_rsd_filter[["feature_included"]] <- "PASS"
master_list$process_lists$statTarget_rsd_filter[["feature_included"]][which(master_list$process_lists$statTarget_rsd_filter$max_rsd>30)] <- "FAIL"
...
master_list$pca$preProcessed$sample_type <- master_list$environment$user_functions$pca$value(
  FUNC_data =  bind_rows(master_list$data$area_impute_concentration_rsd_all, master_list$data$statTarget_impute_concentration_rsd_all) %>%
    select(-contains("SIL")), 
  FUNC_HEADER_run_order = "sample_run_index",
  FUNC_HEADER_plate_id = "sample_plate_id",
  FUNC_HEADER_colour_by = "sample_type_factor",
...
userguideTable <- tibble(
  "userGuide" = data.frame(
    ProjectDetails = c(
      ...
      "DATA_responseRatio",
      "DATA_concentration",
      "DATA_preProcessedConcentration",
      "SUMMARY_PeakArea",
      "SUMMARY_response|concentration",
      "SUMMARY_preProcessedConcentration",
      "FILTER_missingValueSamples",
      "FILTER_missingValueFeatures",
      "FILTER_RSDinQC",
      "",
      "NOTE:"),
    descriptor = c(
      ...
      bind_rows(master_list$data$area_sorted)[["sample_plate_id"]] %>% unique() %>% length(),
      bind_rows(master_list$data$area_sorted) %>% nrow(),
      bind_rows(master_list$data$area_sorted) %>% filter(sample_type_factor == "sample") %>% nrow(),
      bind_rows(master_list$data$area_sorted) %>% filter(sample_type_factor == "ltr") %>% nrow(),
      bind_rows(master_list$data$area_sorted) %>% filter(sample_type_factor == "vltr") %>% nrow(),
      bind_rows(master_list$data$area_sorted) %>% filter(sample_type_factor == "sltr") %>% nrow(),
      bind_rows(master_list$data$area_sorted) %>% filter(sample_type_factor == "pqc") %>% nrow(),
      bind_rows(master_list$data$area_sorted) %>% select(-contains("sample")) %>% select(-contains("SIL")) %>% ncol(),
      bind_rows(master_list$data$area_sorted) %>% select(contains("SIL")) %>% ncol(),
      "",
      "",
      ...
      "concentration data that has undergone pre-processi,ng (missing value sample filter [>50%], missing value feature filter [>50%], imputation remaining missing values [min/2], relative standard deviation filtering [>30%]",
      "sample|feature overview in peakArea dataset",
      "sample|feature overview in response|concentration dataset",
      "sample|feature overview in preProcessedConcentration dataset",
      "sample performance in missing value filter [samples with >50% values <5000 counts removed]",
      "feature performance in missing value filter [features with >50% values <5000 counts removed]",
      "feature performance in rsd filter [features with > 30% RSD across user selected qc are excluded]",
      "",
      "DATA IN THIS SPREADSHEET IS PRODUCED FROM RAW AREA, NO SIGNAL DRIFT OR BATCH CORRECTION HAS BEEN APPLIED"
    )))

## 1. write .ods tabbed file    -------------------------------
write_ods(
  path = paste0(master_list$project_details$project_dir, 
                ...
                "_",
                master_list$project_details$project_name,
                "_lipidData_qcCeheckeR_v3_35.ods"),
  list(
    #uncorrected data
    ...
    #statTarget corrected data
    "ST.data_PeakArea" = bind_rows(master_list$data$statTarget_sorted) %>% select(-contains("SIL")),
    "ST.data_SilPeakArea" = bind_rows(master_list$data$statTarget_sorted) %>% select(contains("sample") | contains("SIL")),
    "ST.data_concentration" = bind_rows(master_list$data$statTarget_concentration),
    "ST.data_preProcessedConcentration" = master_list$data$statTarget_impute_concentration_rsd_all,
    
    #summary
    "SUMMARY_lipidPeakArea" = master_list$summary_tables$area_peakArea,
    "SUMMARY_response|concentration" = master_list$summary_tables$area_response,
    "SUMMARY_preProssedConcentration" = master_list$summary_tables$area_preProcessed,
    "FILTER_missingValueSamples" = master_list$summary_tables$area_mv_filter_samples,
    "FILTER_missingValueFeatures" = master_list$summary_tables$area_mv_filter_lipids,
    "FILTER_RSDinQC" = master_list$process_lists$area_rsd_filter
  )
)
