...
master_list$project_details$qcCheckR_version <- "3.23"
...
statTarget_data <- master_list$environment$user_functions$signal_correct$value(
  ...
  FUNC_data = master_list$data$impute %>% bind_rows() %>% add_column("sample_idx" = 1:nrow(master_list$data$impute %>% bind_rows()), .before = 1),
  FUNC_metabolite_list = master_list$data$impute %>% bind_rows() %>%
    select(!contains("sample")) %>% names(),
...
for(idx_data in names(master_list$data$impute)){
  ...
  master_list$summary_tables$statTarget_table <- master_list$summary_tables$statTarget_table %>%
    bind_rows(
      bind_cols("batch" = idx_data,
                "zero vaules pre-statTarget" = length(which(master_list$data$impute[[idx_data]] %>% select(!contains("sample"))==0)),
                "NA values pre-statTarget" = length(which(is.na(as.matrix(master_list$data$impute[[idx_data]] %>% select(!contains("sample")))))),
                "NaN values pre-statTarget" = length(which(is.nan(as.matrix(master_list$data$impute[[idx_data]] %>% select(!contains("sample")))))), # find NANs
...
master_list$data$concentration_statTarget_bind_plates <- master_list$data$concentration_statTarget %>%
  bind_rows() %>% 
  add_column(sample_idx = seq(1:nrow(bind_rows(master_list$data$concentration_statTarget))),
             .before = 1)
...
write_csv(x = master_list$data$concentration_statTarget_bind_plates,
          file =paste0(
            master_list$project_details$project_dir,
            "/html_report/", Sys.Date(), 
            "_", 
            master_list$project_details$project_name, 
            "_qcCheckeR_v3.23.csv"))

# PROCESS: render html report ---------------------------------------------

fileConn<-file(paste0(master_list$project_details$project_dir, "/html_report/lipid_exploreR_report_templatev3.23.R"))
writeLines(httr::GET(url = paste0(master_list$project_details$github_master_dir, "/templates/TEMPLATE_lipidExploreR_report_v3.23.R")) %>%
             httr::content(as = "text"), fileConn)
close(fileConn)


rmarkdown::render(input = paste0(master_list$project_details$project_dir, "/html_report/lipid_exploreR_report_templatev3.23.R"),
                  output_format = "html_document",
                  output_dir = paste0(master_list$project_details$project_dir, "/html_report"),
                  output_file = paste0(Sys.Date(), "_", master_list$project_details$project_name, "_lipidExploreR_qcCheckeR_report_v3.23.html")
)

browseURL(url = paste0(master_list$project_details$project_dir, 
                       "/html_report/",
                       Sys.Date(), "_", master_list$project_details$project_name, "_lipidExploreR_qcCheckeR_report_v3.23.html")
)
