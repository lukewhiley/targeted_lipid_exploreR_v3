...
rt_boundary_output <- lapply(FUNC_metabolite, function(FUNC_LIPID){
  #browser()
  #print(FUNC_LIPID)
  rt_boundary <-  filter(FUNC_data_qc, molecule_name == FUNC_LIPID) %>% filter(!is.na(area)) %>% filter(!grepl("conditioning", file_name)) %>% arrange(retention_time)
