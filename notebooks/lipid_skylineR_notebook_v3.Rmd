...
master_list$mzR <- list()
master_list$mrm_guides <- list()
master_list$skyline_reports <- list()
...
master_list$project_details$project_dir <- "/Users/lukegraywhiley/Library/CloudStorage/OneDrive-MurdochUniversity/projects/2022-GRAY-LIPID_METHOD/data/lipidomics_data/multiplate_batch/plasma/cambridge_followup_2"
...
#### load packages
package_list <- c('plyr', 'tidyverse', 'janitor', 'shiny', 'plotly', 'effsize', 'dunn.test', 'RColorBrewer', 'statTarget', 'metabom8', 'rstatix', 'ggpubr', 'cowplot', 'knitr', 'stats', 'viridisLite', 'ggbeeswarm', 'q3ML', 'xcms', 'sRm', 'MsCoreUtils', 'svDialogs')
...
```{r read in mzml files, eval = TRUE, echo = FALSE, warning=FALSE, error=FALSE, message=FALSE}
...
for(idx_mzML in mzml_filelist){
  master_list$mzR[[idx_mzML]] <- list()
  master_list$mzR[[idx_mzML]]$mzR_object <- mzR::openMSfile(
    filename = paste0(master_list$project_details$project_dir, 
                      "/data/mzml/",
                      idx_mzML))
  master_list$mzR[[idx_mzML]]$mzR_header <- mzR::chromatogramHeader(master_list$mzR[[idx_mzML]]$mzR_object)
  master_list$mzR[[idx_mzML]]$mzR_chromatogram <- mzR::chromatograms(master_list$mzR[[idx_mzML]]$mzR_object)
  master_list$mzR[[idx_mzML]]$mzR_timestamp <- master_list$mzR[[idx_mzML]]$mzR_object@backend$getRunStartTimeStamp()
}
...
master_list$mrm_data$mrm_guide_rt_update <- master_list$functions$mrm_RT_findeR_mzR$value(
  FUNC_mzR = master_list$mzR, #list for each sample containing $mzR_object; $mzR_header; $mzR_chromatogram
