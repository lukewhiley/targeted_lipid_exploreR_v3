...
#source functions
master_list$functions$mrm_findeR <- source(paste0(master_list$project_details$github_master_dir , "/2022_LGW_FUNC_MRM_findeR.R"))
...
for(idx_package in package_list){suppressMessages(require(package = idx_package, character.only = TRUE))}
rm(list = c(ls()[which(ls() != "master_list")]))
```