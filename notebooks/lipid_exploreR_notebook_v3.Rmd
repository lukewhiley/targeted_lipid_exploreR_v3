...
for(idx_data in names(master_list$data$sorted)){
 ...
 master_list$data$impute[[idx_data]] <- master_list$functions$impute_data(
   FUNC_data = master_list$data$sorted[[idx_data]],
   FUNC_metabolite_list = master_list$data$sorted[[idx_data]] %>% 
     select(-contains("sample")) %>% names(),
...
master_list$pca_output$imputed[[idx_data]] <- lgw_pca(
...
master_list$summary_tables$missing_data_check_2 <- list()
...
master_list$summary_tables$missing_data_check_2 <- names(master_list$data$concentration) %>% tibble() %>% rename("batch" = ".") %>%
...
print(master_list$summary_tables$missing_data_check_2)
...
master_list$pca_output$concentration[[idx_data]] <- lgw_pca(
...
master_list$pca_output$pc_filter[[idx_data]] <- lgw_pca(
...
master_list$summary_tables$missing_data_check_3 <- list()
...
master_list$summary_tables$missing_data_check_4 <- list()
