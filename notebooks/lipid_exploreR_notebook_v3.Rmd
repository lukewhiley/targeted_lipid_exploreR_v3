---
title: "Lipid manuscript - multiplate batch"
output:
  html_document:
    df_print: paged
---
...
- columns for metabolite data with metabolite name as header  - e.g. CE(14:0) ort TRYPTOPHAN or 2.86_180.0882m/z etc

-->

<!-- * USER INPUT REQUIRED FOR HTML REPORT --><!-- load packages and libraries -->

# Project - Lipid Methods paper

Purpose is to evaluate the multi-batch comparison across CABIN, ROCIT and WA COVID cohorts
...
<!-- USER INPUT: set project options -->

```{r, set up project structure, eval = FALSE, echo = FALSE, warning=FALSE, error=FALSE, message=FALSE,  fig.width=1.25, fig.height=1}
#set up project master list
master_list <- list()
master_list$project_details <- list()
master_list$functions <- list()
master_list$tables <- list()
master_list$data <- list()
master_list$pca <- list()
master_list$lists <- list()
...
#set project name
master_list$project_details$project_name <- "lipid paper - multiplate batch"
#set project directory
master_list$project_details$project_dir <- "/Users/lukegraywhiley/Library/CloudStorage/OneDrive-MurdochUniversity/projects/lipid_method_manuscript/multiplate_batch"
...
-> a single csv for each sample plate in the project 
-> should be stored in the project directory, in the subfolder  ~project_directory/data/skyline_exports

-->


```{r, read in skyline data, eval = FALSE, echo = FALSE, warning=FALSE, error=FALSE, message=FALSE,  fig.width=1.25, fig.height=1}
######

master_list$data$skyline <- list()
...
for(idx_data in (data_filelist)){
  #extract filename from path for tidier data storage in list
 idx_data_sub <- gsub(x = strsplit(idx_data, paste0(subfolder, "/"))[[1]][2], 
                         pattern = ".csv", 
                         replacement = "")
  
  master_list$data$skyline[[idx_data_sub]] <- read_csv(paste0(idx_data), show_col_types = FALSE) %>%
    clean_names()
}
...
<!-- SECTION 2: flip data to standard metabolomics structure (features in columns, samples in rows)  

NO USER INPUT NECESSARY -->

### Data summary 1

```{r, transpose data, eval = FALSE, echo = FALSE, warning=FALSE, error=FALSE, message=FALSE,  fig.width=1.25, fig.height=1}

master_list$data$transposed <- list()
master_list$tables$data$transposed_summary <- tibble()

for(idx_data in names(master_list$data$skyline)){
  #create array of unique features
  lipid_features <- master_list$data$skyline[[idx_data]]$peptide %>% unique()
  
  #create array of unique samples in the list
  master_list$data$transposed[[idx_data]] <- 
    master_list$data$skyline[[idx_data]]$replicate %>% 
    unique() %>% 
    as_tibble() %>% 
  ...
  for (idx_lipid in lipid_features){
     master_list$data$transposed[[idx_data]] <-
       left_join(master_list$data$transposed[[idx_data]],
                 master_list$data$skyline[[idx_data]] %>%
...
#make summary table
master_list$tables$data$transposed_summary <- master_list$tables$data$transposed_summary %>% rbind(
  c(idx_data,
...
master_list$tables$data$transposed_summary <- master_list$tables$data$transposed_summary %>%
  setNames(
    c("project", "total samples", "total metabolite features"))

print(master_list$tables$data$transposed_summary)
...
<!-- SECTION: check for missing values in dataset  
section also saves an RDA file for loading at a later date - to do this user must create a subfolder:
~project_directory/data/rda -->

## Checkpoint 1 for missing values

```{r, checkpoint 1 - missing data and save progress, eval = FALSE, echo = FALSE, warning=FALSE, error=FALSE, message=FALSE,  fig.width=1.25, fig.height=1}

master_list$tables$missing_data_check_1 <- list()
...
master_list$tables$missing_data_check_1 <- names(master_list$data$transposed) %>% tibble() %>% rename("batch" = ".") %>%
...
print(master_list$tables$missing_data_check_1)
...
-> write csv out to:
  -> ~project_directory/run_order/r_output
-> user must the manually edit run order and add the following sample headings:
  -> sample_idx - numeric index of the sample
  -> sample_name - file name of each sample
  -> sample_type - string containing "sample" or "qc"
  -> sample_batch - batch descrriptor e.g. project name (WA_COVID etc)
  -> sample_plate_id - numeric descriptor of the plate number
  -> sample_plate_order - numeric descriptor of the run order on the plate
...
# step 1 - write out sample lists

for (idx_data in names(master_list$data$concentration)){
  write_csv(master_list$data$transposed[[idx_data]] %>%
              select(sample_name),
            file = paste0(
          master_list$project_details$project_dir,
          "/run_order/r_output/",
                          idx_data,
                          "_unedited_sample_list.csv"))

}

# step 2 - read in edited sample lists
...
master_list$pca$data_sorted <- list()

#run pca loop
for(idx_data in names(master_list$data$sorted)){
  
master_list$pca$data_sorted[[idx_data]] <- lgw_pca(
...
print(master_list$pca$data_sorted[[idx_data]]$plot_scores)
  }
...
master_list$data$list_missing_value_filter <- list()
master_list$data$missing_value_filter <- list()
master_list$tables$missing_value_filter_summary <- tibble()

for(idx_data in names(master_list$data$sorted)){
   master_list$data$list_missing_value_filter[[idx_data]]$sample_fail_list <- NULL
    master_list$data$list_missing_value_filter[[idx_data]]$feature_fail_list <- NULL
  for(idx_sample_type in c("sample", "qc")){
    master_list$data$list_missing_value_filter[[idx_data]][[idx_sample_type]] <- list()
    master_list$data$list_missing_value_filter[[idx_data]][[idx_sample_type]] <- lgw_missing_value_filter(
      FUNC_data = master_list$data$sorted[[idx_data]] %>%
        filter(sample_type == idx_sample_type),
FUNC_metabolite_list = master_list$data$sorted[[idx_data]] %>%
  select(!contains("sample")) %>% names(),
FUNC_IS_tag = "SIL",
FUNC_OPTION_missing_value_threshold = 50,
FUNC_OPTION_intensity_threshold = 5000)
  
    #bind fail sample list
    master_list$data$list_missing_value_filter[[idx_data]]$sample_fail_list <- c(
      master_list$data$list_missing_value_filter[[idx_data]]$sample_fail_list,
      master_list$data$list_missing_value_filter[[idx_data]][[idx_sample_type]]$mv_samples_fail) %>%
      unique()
    
       #bind fail featire list
    master_list$data$list_missing_value_filter[[idx_data]]$feature_fail_list <- c(
      master_list$data$list_missing_value_filter[[idx_data]]$feature_fail_list,
      master_list$data$list_missing_value_filter[[idx_data]][[idx_sample_type]]$mv_features_fail) %>%
      unique()
    }

  #select only pass samples
  master_list$data$missing_value_filter[[idx_data]] <- master_list$data$sorted[[idx_data]] %>%
    filter(!sample_name %in% master_list$data$list_missing_value_filter[[idx_data]]$sample_fail_list) %>%
    #select only pass features
    select(-all_of(master_list$data$list_missing_value_filter[[idx_data]]$feature_fail_list))
#   
#   
  master_list$tables$missing_value_filter_summary <-  master_list$tables$missing_value_filter_summary %>%
    bind_rows(
...
# 
print(master_list$tables$missing_value_filter_summary)
#clean environment
rm(list = c(ls()[setdiff(which(ls() != "master_list"), which(ls() %in% c(lsf.str())))]))
```
...
#create empty list for results
master_list$pca$missing_value_filter <- list()

#run pca loop
for(idx_data in names(master_list$data$missing_value_filter)){
  
master_list$pca$missing_value_filter[[idx_data]] <- lgw_pca(
...
print(master_list$pca$missing_value_filter[[idx_data]]$plot_scores)
  }
...
master_list$tables$impute_table <- tibble()

for(idx_data in names(master_list$data$sorted)){
  
 master_list$data$impute[[idx_data]] <- lgw_lipid_impute(FUNC_data = master_list$data$sorted[[idx_data]],
                    FUNC_metabolite_list = master_list$data$sorted[[idx_data]] %>% 
                    select(-contains("sample")) %>% names(),
                  FUNC_option_impute_missing_data = TRUE)

#create summary tibble
master_list$tables$impute_table <-  master_list$tables$impute_table %>%
  bind_rows(
  bind_cols("batch" = idx_data,
...
print(master_list$tables$impute_table)
...
#create empty list for results
master_list$pca$imputed <- list()

#run pca loop
for(idx_data in names(master_list$data$impute)){
  
master_list$pca$imputed[[idx_data]] <- lgw_pca(
...
print(master_list$pca$imputed[[idx_data]]$plot_scores)
  }
...
for(idx_data in names(master_list$data$impute)){
  master_list$data$concentration[[idx_data]] <- lgw_lipid_conc_calc(FUNC_data = master_list$data$impute[[idx_data]],
 FUNC_metabolite_list = master_list$data$impute[[idx_data]] %>% 
   select(!contains("sample")) %>% names(),
 FUNC_SIL_guide_path =  paste0(master_list$project_details$project_dir,"/sciex_templates/2021-09-21-LGW_lipid_mrm_transition_data.csv"),
 FUNC_conc_guide_path = paste0(master_list$project_details$project_dir, "/sciex_templates/2021-LGW_SIL_batch_103.csv"))
}
...
```{r, checkpoint 2 - missing data and save progress after conc calc, eval = FALSE, echo = FALSE, warning=FALSE, error=FALSE, message=FALSE,  fig.width=1.25, fig.height=1}

master_list$tables$missing_data_check_2 <- list()
...
master_list$tables$missing_data_check_2 <- names(master_list$data$concentration) %>% tibble() %>% rename("batch" = ".") %>%
  cbind(temp_missing_data %>% as_tibble() %>% setNames(c("zero values", "NA values", "NaN values"))); 
...
print(master_list$tables$missing_data_check_2)
...
master_list$pca$concentration[[idx_data]] <- lgw_pca(
...
print(master_list$pca$concentration[[idx_data]]$plot_scores)
  }
...
master_list$tables$pc_filter_summary <- tibble() # sample summary post filtering
...
master_list$data$list_pc_filter[[idx_data]] <- lgw_pca_filter(
  FUNC_data = master_list$data$concentration[[idx_data]],
  FUNC_metabolite_list = master_list$data$concentration[[idx_data]] %>%
      select(-contains("sample")) %>% names(),
  FUNC_colour_by = "sample_type_factor",
...
master_list$tables$pc_filter_summary <- master_list$tables$pc_filter_summary %>% rbind(
  c(idx_data,
...
master_list$tables$pc_filter_summary <- master_list$tables$pc_filter_summary %>%
...
print(master_list$tables$pc_filter_summary)
...
#create empty list for results
master_list$pca$pc_filter <- list()

#run pca loop
for(idx_data in names(master_list$data$pc_filter)){
  
master_list$pca$pc_filter[[idx_data]] <- lgw_pca(
...
print(master_list$pca$pc_filter[[idx_data]]$plot_scores)
  }
...
master_list$tables$missing_data_check_3 <- list()
...
master_list$tables$missing_data_check_3 <- names(master_list$data$pc_filter) %>% tibble() %>% rename("batch" = ".") %>%
  cbind(temp_missing_data %>% as_tibble() %>% setNames(c("zero values", "NA values", "NaN values"))); 
...
print(master_list$tables$missing_data_check_3)
...
for (idx_data in names(master_list$data$pc_filter)){
  ...
  master_list$data$corrected_individual_batch[[idx_data]] <- lgw_signal_correction(
    FUNC_project_directory = paste0(master_list$project_details$project_dir,
                                    "/per_project_batch_correction/",
                                    idx_data),
    FUNC_data = master_list$data$pc_filter[[idx_data]],
    FUNC_metabolite_list = master_list$data$pc_filter[[idx_data]] %>%
      select(-contains("sample")) %>% names(),
...
master_list$tables$missing_data_check_4 <- list()
...
for(idx_data in names(master_list$data$corrected_individual_batch)){
  ...
  master_list$tables$missing_data_check_4[[idx_data_2]] <- names(master_list$data$corrected_individual_batch) %>% tibble() %>% rename("batch" = ".") %>%
  cbind(temp_missing_data %>% as_tibble() %>% setNames(c("zero values", "NA values", "NaN values")))
}

# master_list$tables$missing_data_check_4 <- names(master_list$data$corrected_individual_batch) %>% tibble() %>% rename("batch" = ".") %>%
#   cbind(temp_missing_data %>% as_tibble() %>% setNames(c("zero values", "NA values", "NaN values")))
...
print(master_list$tables$missing_data_check_4)
...
#create empty list for results
master_list$pca$corrected_individual_batch <- list()

#run pca loop
for(idx_data in names(master_list$data$pc_filter)){
  for(idx_data_2 in names(master_list$data$corrected_individual_batch[[1]])){
  
master_list$pca$corrected_individual_batch[[idx_data]][[idx_data_2]] <- lgw_pca(
...
#print(master_list$pca$corrected_individual_batch[[idx_data]][[idx_data_2]]$plot_scores)
}
  print(master_list$pca$corrected_individual_batch[[idx_data]][[idx_data_2]]$plot_scores)
}
...
master_list$tables$missing_data_check_6 <- tibble()
...
for(idx_data_2 in names(master_list$data$all_batches_combined$batch_corrected)){
  ...
  master_list$tables$missing_data_check_6 <- bind_rows(master_list$tables$missing_data_check_6,
    idx_data_2 %>%
  cbind(temp_missing_data %>% as_tibble() %>% setNames(c("zero values", "NA values", "NaN values")))
  )
}

print(master_list$tables$missing_data_check_6 )
...
master_list$tables$final_data_summary <- tibble()

 master_list$tables$final_data_summary <- master_list$tables$final_data_summary %>%
    rbind(
  c(
    "total datset",
...
print(master_list$tables$final_data_summary)
...
#create empty list for results
master_list$pca$final_dataset <- list()

#before batch correction

#create PCA
master_list$pca$final_dataset$pre_correction <- lgw_pca(
  FUNC_data = master_list$data$all_batches_combined$data,
  FUNC_metabolite_list = master_list$data$all_batches_combined$data %>%
      select(-contains("sample")) %>% names(),
  FUNC_colour_by = "sample_type_factor",
...
print(master_list$pca$final_dataset$pre_correction$plot_scores)

for(idx_data_2 in names(master_list$data$all_batches_combined$batch_corrected)){

#create PCA
master_list$pca$final_dataset[[idx_data_2]] <- lgw_pca(
...
#print(master_list$pca$final_dataset[[idx_data_2]]$plot_scores)
}
...
master_list$pca$final_dataset$pre_correction$plot_scores
master_list$pca$final_dataset$concentration_data$plot_scores
