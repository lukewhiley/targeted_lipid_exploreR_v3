...
for(idx_data in names(master_list$data$skyline_reports)){
  #create array of unique features
  lipid_features <- master_list$data$skyline_reports[[idx_data]]$peptide %>% unique()
  
  #create array of unique samples in the list
  master_list$data$transposed[[idx_data]] <- 
    master_list$data$skyline_reports[[idx_data]]$replicate %>% 
    unique() %>% 
    as_tibble() %>% 
  ...
  for (idx_lipid in lipid_features){
     master_list$data$transposed[[idx_data]] <-
       left_join(master_list$data$transposed[[idx_data]],
                 master_list$data$skyline_reports[[idx_data]] %>%
...
master_list$data$skyline_reports <- list()

#add data subfolder here
subfolder <- "/data/skyline_reports"
...
for(idx_data in (data_filelist)){
 ...
 #read in each skyline export in ~parent_directory/data/skyline_reports sub-folder
  master_list$data$skyline_reports[[idx_data_sub]] <- read_csv(paste0(idx_data), show_col_types = FALSE) %>%
    clean_names()
}
